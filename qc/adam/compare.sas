...
*  29MAY2023   | Stuart Malcolm | Embedded macro and change paths
\*****************************************************************************/
...
*
*  Batch TFL call via FINAL.SAS program (TFL)
...
*  23May2022     | Mark Holland | Added functionlaity when TFL=Y to drop ordering vars from compare if present
*  15Jul2022     | Mark Holland | option to allow checks against datasets of only a given name (string after prefix) NAMEPRE.
*  28Sep2022     | Mark Holland | options=listall set to default to ensure all unique vars and info listed
*  10Oct2022     | Mark Holland | Add PAGE to the list of variables that can be deleted from one PROD data in TFL compare
*                                 Also for DROP vars, split characters supression, only process in BASE (production) data
*  27Oct2022     | Mark Holland | Added OUTNOEQUAL option in sort to check sort status of IDVAR check 
*  28Oct2022     | Mark Holland | Allowed ROWORDn vars also to be dropped for TFL data (previously just ROWORD)
*  28Oct2022     | Mark Holland | Allowed ROWORDn vars also to be dropped for TFL data (previously just ROWORD)
*  08Nov2022     | Mark Holland | Criterion option corrected in compare call when present
\*****************************************************************************/
...
%macro s_compare(
    ...
    options=listall,            /* [Not Required]: compare options to apply to PROC COMPARE statement */
    ...
    TFL=N,                      /* [Required] : Pre-fills IgnoreMask values with the recommended masks for TFL checking */
                                /*              and also defaults COMPBL to text variables
                                                also drops pre-approved varilables from BASE side */
    splitchar=,                 /* [Not Required]: The character which is used to split labels/character string
                                                   ...
                                                   Removal is only from the dataset on the BASE side */
    ...
    namepre=            /* Allows selection of datasets only starting with a specific string.  */ 
                        /* Useful if wanting to run only against T F or L datasets only  */
    );
    ...
    %macro ___precomp(type=);
       ...
       %let lblchgn=0;  ** forced in case no num vars **;
        ...
        %if &TFL=Y %then %do;
            ...
            %let dsid = %sysfunc(open(&&&type));
             %** 28Oct2022.  Mark H.  Added ROWORD1 into to valid drop list **;
            %if %sysfunc(varnum(&dsid, ROWORD))  gt 0 or %sysfunc(varnum(&dsid, ROWORD1)) %then %let __DROPVARS= &__DROPVARS ROWORD:;;
            ...
            %if %sysfunc(varnum(&dsid, PAGE))    gt 0 %then %let __DROPVARS= &__DROPVARS PAGE;; ** 10Oct2022.  Mark H.  PAGE added as drop variable **;
        ...
        data _&type.;
          set &&&type.  %if &type=base %then  &__drop;; ** 07Oct2022 Mark H Drop allowed vars from BASE only **;
        run;
        ...
        * Check that there are at least some character variables on the input dataset,
            ...
            * Also, apply any split character, space removal (entirely or just repeated spaces) here;
              ...
              do __ii__ = 1 to dim(_charall_);
                ...
                if ^missing("&splitchar") then do;
                   ...
                   %** 10Oct2022 Mark H Only process data on BASE side.  This with be PROD **;
                   %** it is assumed these vars will not exist COMP (QC)                   **;
                ...
                %end;
                  ...
                  * Apply modifications to the label also;
                  %* Label is processed, but only applied below if type=BASE **;
                 ...
                 ** Data compression applied to both BASE and COMP **;
                if "&compbl" = "Y" then do;
        ...
        proc datasets lib=work mt=data nolist nodetails;
           ...
           ** 10Oct2022 Mark H Only do this however for BASE (PROD) execution **;
        %if &type=base %then %do;
...
/*        run;*/
        ...
        %if %nrbquote(&idvars.) ne %then %do;
           ...
           **27Oct2022 Mark holland - Added OUTNOEQUAL so that if match created dataset is null **;
          proc compare base = &type._presorted c = _&type. noprint out=_&type.sortchk outnoequal;
          run;
...
%put _addstr3=&_addstr3;
    ...
    ** If NAMEPRE is set, then add string into title for T3 statement;
    %global namettl;
    %if &namepre ne %str() %then %let namettl=(&namepre.---- datasets only);
    %else %let namettl=;

    * Final proc compare section;
    * 09Nov2022 Mark H.  correction to criterion call when present *;
...
%if %index(%upcase(&base),_ALL_) %then %do;
       ...
       data ___libbase (rename=(libname=baselib crdate=baseDTM))
          ...
          if libname="&___complib" then do;
                 ...
                 %if %str(&namepre) ne %str() %then %do;
                    if upcase(substr(memname,1,length(trim(left("&namepre"))))) = upcase(trim(left("&namepre")));
                 %end;
                 output ___libcomp;
          end;
       ...
       run;
        ...
        %** Set local var is zero datasets processed **;
       %if &dsetnum=0 %then %let namettl=;
      ...
      %do i = 1 %to &dsetnum;
         ...
         %___COMP;
          ...
          %** also add in dataset containing datasets in one side only **;
            data ___libALLcomp;
               set ___libALLcomp
      ...
      ODS PDF file="%str(&comprpt)";
          ...
          title3 "Summary of Dataset Compare &namettl.";
...
%s_compare(base = ADAM._ALL_,
		   comp = ADAMQC._ALL_,
		   comprpt = '/mnt/artifacts/results/compare.pdf',
		   prefix =,
		   tidyup = N);
